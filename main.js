...
fetch(BREEDS_URL)
    ...
    .then(data => {
        ...
        const breedsArray = Object.keys(breedsObject) //Turn the Object into Array
        for (let i=0; i<breedsArray.length; i++) {
            const option = document.createElement('option') // <option></option>
            option.value = breedsArray[i] //<option value="breed">
            option.innerText = breedsArray[i] //add value of breed to option tag <option value="breed">breed</option>
            select.appendChild(option) //create list of selectable options by adding current <option>

        }
    })

    // Add event listener when you click the breed type it shows u the img 
    select.addEventListener('change', event => {
        //console.log(event.target.value) -> when you select the breed, it console logs that breed type
        let url = `https://dog.ceo/api/breed/${event.target.value}/images/random`
        //console.log(url) // when breed is selected, the breed url to the json data gets consoled
        //need a function that gets the img of the breed
        getDoggoImg(url)
        doggoInfo.assignMF() //method inside dogooInfo object
        doggoInfo.assignAge()
        doggoInfo.assignLikes()
        doggoInfo.assignDislikes()
        doggoInfo.assignFunFact()
    })
    //declare variable that targets the img tag
    const img = document.querySelector('.dog-img') //replace contents of img tag with new img
    ...
    const getDoggoImg = url => {
        fetch(url) //going to the API url above
            ...
            .then(data => {
                img.src = data.message //extract message from JSON & attach to img tag as new source
    ...
    const doggoInfo = {
        ...
        //randomly assign gender using Math.random()
        //use a method assignMF(){}
        assignMF(){
            x = (Math.floor(Math.random() *2) == 0) // Math.random() result is either 0 or 1. Times 2 now: between 0 and 2. Math.floor rounds down to next lowest integer (from 2 to 1). So results is going to be either 0 or 1 and compares it to 0.
            if (x) {
                this.MF  = "Female";
            ...
            //assign result of assignMF() call S: Male or S: Female
            document.getElementById("MF").innerHTML = `S: ${this.MF}`
        },
        //get back name of dog from array. Name is being called in assignMF()
        assignName(array) {
        ...
        assignAge(){
            this.age = Math.floor(Math.random() * 16 + 1) //return random bw 1-16
      ...
      return array;
        ...
        assignLikes() {
            this.likes = this.yatesShuffle(this.likesList).slice(0,2) //get random 2 elements (likes)
